...
const BookingFlexibleByPackage = () => {
  ...
  useEffect(() => {
    const fetchCenterDetails = async () => {
      try {
        ...
        message.error(
          'Đã xảy ra lỗi khi lấy thông tin trung tâm. Vui lòng thử lại sau.'
        );
      }
    };
  ...
  const getPriceForScheduleType = () => {
    if (
      ...
      center.data &&
      center.data.center &&
      center.data.center.price
    ) {
      const priceObject = center.data.center.price.find(
        (price) => price.scheduleType === 'PP'
      );
